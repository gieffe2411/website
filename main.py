...
def rotta_testlunga():
    ...
    CHARSET = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZàèéìòù ,.?!'"
    ...
    def generate():
        ...
        while index < len(TARGET):
            ...
            yield f"Iterazione {iteration} — Tentativo: '{char}'<br>\n"
            time.sleep(0.005)

            if char == TARGET[index]:
                risultato += char
                index += 1
                yield f"<b>OK</b> → {risultato}<br><br>\n"

        yield "<br><b>TROVATA!</b><br>"
        yield f"Testo: {risultato}<br>"
